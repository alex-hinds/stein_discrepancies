...
class multivariate_t_dist:  # multivariate t distribution with isotropic scale matrix
    ...
    def __init__(self, loc, scale, df):
        ...
        self.Gaussian = torch.distributions.MultivariateNormal(
            torch.zeros(p), scale * torch.eye(p)
        )
    ...
    def log_density(self, x):
        ...
        A1 = (
            gammaln((self.df + p) / 2)
            - gammaln(self.df / 2)
            - (p / 2) * (np.log(self.df) + np.log(math.pi) + np.log(self.scale))
        )
    ...
    def sample(self, M):
        ...
        return torch.stack(
            [a1[j] / torch.sqrt(a2[j] / self.df) + self.loc for j in range(M)]
        )
...
class imq_ksd:
    ...
    def L1_RF_sd(self, q_samples, score_p, M, gamma=0.25, target_df=3, n_list=None):
