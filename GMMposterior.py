...
class GMM_posterior():
    ...
    def __init__(self, theta_1, theta_2, R, sigma2_1 = 1., sigma2_2 = 10., sigma2_x = 2.):
    ...
    def prior_score(self, theta_vec):
        ...
        prior_dist = torch.distributions.MultivariateNormal(torch.zeros([2]), torch.diag(sigmas))
    ...
    def make_GMM_density(self, theta_vector, r):
        ...
        def GMM_density(theta_vec):
            mixture_1 = 1/(2*math.pi*sigma2_x)**0.5 * torch.exp( - (x - theta_vec[0])**2 / (2*sigma2_x))
            mixture_2 = 1/(2*math.pi*sigma2_x)**0.5 * torch.exp( - (x - theta_vec[0] - theta_vec[1])**2 / (2*sigma2_x))
    ...
    def log_posterior_density(self, theta_vec):
        ...
        log_prior = torch.distributions.MultivariateNormal(torch.zeros([2]),torch.diag(sigmas)).log_prob(theta_vec)
                
        lik_list = [torch.log(self.make_GMM_density(theta_vec, r)) for r in range(self.R)]
        log_lik_sum = 0
    ...
    def make_posterior_score(self, theta_vec, r):  # decompose score into r terms (for Stoch KSD)
        return 1/self.R * self.prior_score(theta_vec) + self.make_lik_grad(theta_vec, r)
    
    
    def posterior_score(self, theta_vec):
        return self.prior_score(theta_vec) + sum([self.make_lik_grad(theta_vec, r) for r in range(self.R)]) 
