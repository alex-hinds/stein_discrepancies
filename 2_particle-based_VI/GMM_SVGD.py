...
init_samples_SVGD = torch.distributions.MultivariateNormal(
    torch.zeros(2), torch.eye(2)
).sample([n_particles])
theta_new_SVGD = init_samples_SVGD.requires_grad_()


init_samples_stochSVGD = torch.distributions.MultivariateNormal(
    torch.zeros(2), torch.eye(2)
).sample([n_particles])
...
for iter in range(n_iter + 1):
    ...
    if iter % 5 == 0:
        ...
        for ind in range(2):
            ...
            Z = np.array(
                [
                    log_posterior_density(torch.tensor(xx, dtype=torch.float32))
                    .detach()
                    .numpy()
                    for xx in XX
                ]
            ...
            plt.scatter(
                theta_new[:, 0].detach().numpy(),
                theta_new[:, 1].detach().numpy(),
                marker="x",
                c="black",
            )
            plt.xlabel(r"$\theta_1$")
    ...
    theta_new_stochSVGD = GMM_SVGD.stoch_update(
        theta_old_stochSVGD, make_score_p, n_steps, int(0.1 * R), R
    )

    print(
        "iter {} out of {} done, time = {}".format(
            iter + 1, n_iter, time.time() - start_time
        )
    )
